...
**驗收條件**：
- [ ] 呼叫 `saveGeminiApiKey('AIza...')` → `api-keys.json` 有 `gemini.apiKey` 欄位
- [ ] 呼叫 `readGeminiKeyStore()` → 回傳 `{ apiKey: '...' , updatedAt: '...' }`
- [ ] 空 key 呼叫 `getRequiredGeminiApiKey()` → 拋出含「尚未設定 Gemini API key」的錯誤
...
**驗收條件**：
- [ ] 傳入合法 prompt → 回傳 parsed JSON
- [ ] 傳入無效 key → 拋出包含 HTTP 狀態碼的錯誤
...
**驗收條件**：
- [ ] curl `GET /api/gemini/settings` → JSON 含 `configured: false`（未設定時）
- [ ] curl `POST /api/gemini/key` with `{ "apiKey": "AIza..." }` → 200 `{ ok: true }`
...
**驗收條件**：
- [ ] 正常輸入 → 200 回傳 3~7 個候選
- [ ] 未設定 key → 400 `{ error: '尚未設定 Gemini API key' }`
- [ ] text 超過 50000 字 → 400 `{ error: '內容過長...' }`
- [ ] Gemini 回傳非 JSON → 500 `{ error: 'AI 回傳格式錯誤...' }`
...
**驗收條件**：
- [ ] 開啟設定頁，可看到 Gemini API Key 區塊
- [ ] 輸入 key 後按儲存 → 狀態更新為「已設定」
- [ ] 按清除 → 狀態回「未設定」
...
**驗收條件**：
- [ ] 頁面顯示新按鈕，樣式與現有按鈕一致
- [ ] Gemini key 未設定時，點擊後顯示「請先在設定頁設定 Gemini API key」提示
...
**驗收條件**：
- [ ] 點擊 AI 提取按鈕 → loading 顯示 → 候選出現
- [ ] 候選卡片顯示「AI 建議」來源標籤
- [ ] 可正常 adopt / reject，並 writeback（使用現有邏輯）
...
**測試項目**：
- [ ] 貼入 `0resource/gemini_20260410.txt` 內容 → AI 輔助提取 → ≥ 3 個有意義候選
- [ ] 採用率（採用 / 總候選）≥ 50%
- [ ] 既有 Regex 提取按鈕仍正常運作
- [ ] 設定頁 Gemini key 儲存 / 顯示 / 清除功能正常
- [ ] 未設定 key 時點擊 AI 提取 → 正確錯誤提示
