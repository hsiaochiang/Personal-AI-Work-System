...
Preserve the existing plain-text `/extract` workflow while routing it through a shared `PlainTextAdapter` that emits a valid `ConversationDoc` and feeds the extraction pipeline through the V3 schema boundary.
