# memory-dedup-suggestions

## Purpose

Provide a safe first-pass dedup governance flow for `/memory` by surfacing duplicate suggestions within the same memory file, preserving existing raw content / health / source consumers, and requiring user-confirmed merge or delete actions with backup protection.
...
#### Scenario: merge action 僅能接受同一 duplicate group 的 primary item
- **WHEN** 使用者或 client 對 dedup merge action 傳入不屬於本次 duplicate group 的 `primaryItemId`
- **THEN** 系統 MUST 拒絕該操作
- **AND** 不得改寫同檔案內其他非目標條目
...
- **AND** 驗證 MUST 覆蓋 malformed merge payload rejection 與 `/memory` 頁面引用 dedup utility / suggestion container / action wiring 的契約
