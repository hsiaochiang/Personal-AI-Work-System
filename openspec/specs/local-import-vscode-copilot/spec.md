...
Allow `/extract` to ingest a single VS Code GitHub Copilot Chat session directly from local JSONL storage, so the user can load a recent session without copy-paste while preserving the existing ChatGPT and plain-text flows.
