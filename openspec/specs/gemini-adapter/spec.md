...
Allow `/extract` to accept pasted Gemini transcripts as a first-class import source, normalize them into `ConversationDoc` with `source: gemini`, and preserve that provenance through candidate review and memory writeback without regressing existing `plain`, `chatgpt`, or `copilot` flows.
