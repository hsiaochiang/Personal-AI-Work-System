...
Allow `/extract` to accept pasted Claude transcripts as a first-class import source, normalize them into `ConversationDoc` with `source: claude`, and preserve that provenance through candidate review and memory writeback without regressing existing `plain`, `chatgpt`, `gemini`, or `copilot` flows.
