# chatgpt-api-auto-import

## Purpose

Allow `/settings` to store an OpenAI API key server-side only, let `/extract` load tracked OpenAI platform conversations inside ChatGPT import mode, and preserve `chatgpt-api` provenance through candidate review and memory writeback without claiming access to ChatGPT product chat history.
...
### Requirement: System MUST list and load only tracked OpenAI platform conversations
...
#### Scenario: Reject direct load for untracked conversation
- **WHEN** 使用者或 client 直接呼叫 `/api/chatgpt/session` 請求一個尚未被目前 project 追蹤的 `conversationId`
- **THEN** server MUST 拒絕該請求
- **AND** server MUST NOT 隱式建立 tracked record
