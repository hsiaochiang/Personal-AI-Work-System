...
- **使用者可見影響**：有
- **影響摘要**：`/extract` 頁面新增 ChatGPT transcript auto-detect 與 `.json` / `.txt` 檔案上傳；既有純文字貼上流程不變，工具來源選擇與 source badge 仍待後續 changes
- **需同步更新的 `docs/system-manual.md` 區段**：
  - 「知識提取與寫回（/extract）」— 說明 ChatGPT transcript / JSON 與 plain-text fallback
  - 「已知限制」— 說明多 conversation deterministic 選擇與未實作項目
  - 「版本歷史摘要」— 加入 V3 完成記錄
