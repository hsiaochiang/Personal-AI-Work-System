...
| `plain-text-adapter-refactor` | 身為使用者，我想要貼上純文字的流程繼續和以前一樣工作，以便 V3 升級不破壞我現有的使用習慣 | `#opsx-archive` 已完成（2026-04-03） | 將現有純文字貼上邏輯抽取為 `PlainTextAdapter`，輸出 `ConversationDoc`；為後續 adapter 建立介面基線；extraction engine 改為接受 `ConversationDoc` 而非裸文字 | `http://localhost:3000/extract` → 純文字貼上 → 流程不變 |
| `chatgpt-adapter` | 身為在 ChatGPT 工作的使用者，我想要把 ChatGPT 的對話複製貼上或上傳，讓系統自動識別並提取知識，以便不用手動清理格式就能把 ChatGPT 的思考結果存進專案記憶 | `#opsx-archive` 已完成（2026-04-03） | 實作 `ChatGPTAdapter`：解析 ChatGPT 共享頁面複製格式與 JSON 匯出格式；本輪採 auto-detect + 最小上傳入口，不擴大到 multi-source selector；輸出 `ConversationDoc`（source: `chatgpt`） | `http://localhost:3000/extract` → 貼上 ChatGPT transcript，或上傳 `.json` / `.txt` 後提取 |
| `local-import-vscode-copilot` | 身為每天使用 VS Code GitHub Copilot Chat 的使用者，我想要不用複製貼上，直接從本機自動讀取 Copilot Chat 的對話記錄，以便讓開發過程的決策與知識能用最少摩擦流進記憶系統 | `#opsx-archive` 已完成（2026-04-03） | 自動讀取本機 Copilot session JSONL；列出最近 session 供選擇；輸出 `ConversationDoc`（source: `copilot`）；路徑設定可由使用者覆寫 | `/extract` → 刷新 Copilot session → 選 session → 載入後提取候選 |
| `source-attribution-in-memory` | 身為需要追溯知識來源的使用者，我想要在記憶條目旁看到「這是從哪個 AI 工具的對話提取的」，以便判斷這條記憶的可信度與背景脈絡 | `#opsx-archive` 已完成（2026-04-03） | writeback 時在 memory markdown 條目加入 `<!-- source: chatgpt -->` metadata；memory 頁面解析並顯示來源小 badge（純色標籤，不破壞現有列表樣式）；本輪不擴大到 import UI selector | `/memory` → 每條記憶旁顯示來源 badge |
| `import-ui-multi-source` | 身為每天使用多個 AI 工具的使用者，我想要在匯入頁面選擇對話來源工具，不論是本機自動讀取還是手動上傳貼上，以便讓系統自動用正確的方式解析，而不是每次都要確認格式是否正確 | `#opsx-archive` 已完成（2026-04-04） | `/extract` 頁面改版：保留既有純文字貼上區域；新增「工具來源」下拉選單（VS Code Copilot / ChatGPT / 純文字）；VS Code 選項顯示本機 session 列表（自動讀取）；ChatGPT 選項顯示貼上 + `.json` 上傳入口；根據選擇自動呼叫對應 adapter；候選審核需顯示來源標記 | `/extract` → 選工具（VS Code → 選 session；ChatGPT → 貼上/上傳；Plain → 直接貼文字）→ 提取候選 |
...
- **使用者可見影響**：有
- **影響摘要**：`/extract` 現在有 `plain` / `chatgpt` / `copilot` 工具來源 selector，會依所選來源顯示對應匯入控制，候選審核卡片也會顯示來源 badge；既有 memory source metadata / `/memory` badge 持續保留
- **需同步更新的 `docs/system-manual.md` 區段**：
  - 「知識提取與寫回（/extract）」— 說明工具來源 selector、per-source controls 與候選來源 badge
  - 「專案記憶（/memory）」— 說明來源 badge 顯示與 legacy 相容策略
  - 「已知限制」— 說明目前 selector 只涵蓋 `plain` / `chatgpt` / `copilot`
  - 「版本歷史摘要」— 加入 V3 Change 6 完成記錄
