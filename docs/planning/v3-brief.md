...
| `local-import-vscode-copilot` | 身為每天使用 VS Code GitHub Copilot Chat 的使用者，我想要不用複製貼上，直接從本機自動讀取 Copilot Chat 的對話記錄，以便讓開發過程的決策與知識能用最少摩擦流進記憶系統 | `#opsx-sync` 已完成（2026-04-03；待 archive） | 自動讀取本機 Copilot session JSONL；列出最近 session 供選擇；輸出 `ConversationDoc`（source: `copilot`）；路徑設定可由使用者覆寫 | `/extract` → 刷新 Copilot session → 選 session → 載入後提取候選 |
...
- **使用者可見影響**：有
- **影響摘要**：`/extract` 頁面新增 Copilot 本機 session list、path override 與單一 session 載入；既有 ChatGPT transcript / JSON 匯入與 plain text fallback 維持可用，工具來源 selector 與 source badge 仍待後續 changes
- **需同步更新的 `docs/system-manual.md` 區段**：
  - 「知識提取與寫回（/extract）」— 說明 Copilot local import、ChatGPT transcript / JSON 與 plain-text fallback
  - 「已知限制」— 說明 Copilot 單一路徑掃描 / 單筆載入與未實作項目
  - 「版本歷史摘要」— 加入 V3 Change 4 完成記錄
