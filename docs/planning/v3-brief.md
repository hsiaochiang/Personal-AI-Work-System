...
- **使用者可見影響**：有
- **影響摘要**：`/extract` 寫回 memory 時會保存 `plain` / `chatgpt` / `copilot` 來源 metadata，`/memory` 會對新寫回或手動補 metadata 的條目顯示來源 badge；既有 import 流程維持不變，工具來源 selector 仍待後續 Change 6
- **需同步更新的 `docs/system-manual.md` 區段**：
  - 「知識提取與寫回（/extract）」— 說明 writeback 會保存 source metadata
  - 「專案記憶（/memory）」— 說明來源 badge 顯示與 legacy 相容策略
  - 「已知限制」— 說明歷史 memory 條目不會自動回填 metadata
  - 「版本歷史摘要」— 加入 V3 Change 5 完成記錄
