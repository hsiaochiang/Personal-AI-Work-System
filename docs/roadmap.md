...
| **V6** | 記憶 AI 策展層 | 對記憶條目執行刪除、讓 AI 整理分類、KPI 可點擊篩選問題條目 | 刪除 + AI Curate + 篩選 | 🔄 進行中 |
...
## 3. 當前版本：V6 進行中 — 記憶 AI 策展層

> **Current development version：V6**
> **Current release：V5（v1.1.0）**
> **Development status：V6 `memory-ai-curator` 已完成 implementation commit + push，待 `/opsx-sync`**
...
V1–V5 已完成；工作台現已具備多來源匯入、memory 治理、規則衝突提示、跨專案 shared knowledge 候選與 Overview 治理待辦。V6 正在把 `/memory` 從只讀治理頁升級成可操作的策展介面；`memory-ai-curator` 已完成 Review Gate、implementation commit 與 push，下一步是執行 main spec sync，再由人決定是否 archive。
...
| # | Change | 狀態 | 備註 |
|:-:|--------|:----:|------|
| 1 | memory-ai-curator | 🔄 進行中 | T-01~T-06、strict validate、targeted verify、ephemeral API smoke、UI/UX review、Review Gate、implementation commit 與 push 已完成；待 `/opsx-sync` |
