...
| **V5** | 外部 API 整合與多工具擴充層 | 以 Gemini / Claude 半自動 adapter 補齊工具覆蓋；透過 ChatGPT API key 自動抓取對話，讓知識匯入更完整 | Gemini/Claude adapter + ChatGPT API auto-import | 🔄 進行中 |
...
**版本目標**：讓工作台的對話知識輸入管道從「手動主導」往「外部自動化」推進一步：補齊 V3 遺留的 Gemini / Claude 半自動 adapter，並引入 API key 機制支援 ChatGPT 對話自動抓取。

**核心能力**：
- Gemini 對話貼上 adapter（半自動）
- Claude 對話貼上 adapter（半自動）
- ChatGPT API auto-import（需使用者提供 API key）
...
- **下一步：依 V5 brief 順序啟動 `claude-adapter`，先執行 `docs/agents/codex-prompts/v5/04-claude-adapter-plan.md`**
