...
### 步驟 D：寫入 Gate 報告至 `openspec/changes/<name>/gate.md`

**無論 PASS 或 BLOCKED，均必須建立或覆寫此檔案**（作為 `#opsx-archive` 的硬閘依據）。

格式：

````markdown
# Gate Report — <change-name>

- **日期**：<YYYY-MM-DD HH:MM>
- **Change 類型**：<ui / logic / doc-only>
- **最終判定**：<PASS / BLOCKED>
...
（無，或列出已由使用者聲明的 Override 項目與原因）
````

判定規則：
- **PASS**：所有「必需」項目均為 ✅（`—` 不適用不影響判定）
- **BLOCKED**：任一「必需」項目為 ❌ 且無 Override 聲明

> `gate.md` 是 verify 交給 archive 的憑據文件；archive 讀取此文件，不重複掃描。
