...
- 有本次的工作成果（git diff --stat 或需知道本次做了什麼）
...
## 禁止事項
- 在任務進行將 session 當式普通已完成（只在最後階段呼叫）
- 版湊 `docs/runlog/` 找把 `slides outline` 拿來充數（內容不同）
- 略過 Checklist 而直接產出 slides（臨陣必須先修夢文件）
