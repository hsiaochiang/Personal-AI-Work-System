...
### V6：記憶 AI 策展層（Memory AI Curator）

**新增**
- **單條刪除記憶**：`/memory` 每個條目新增「刪除」按鈕，刪除前自動備份至 `.backup/`，不可逆操作需二次確認（`POST /api/memory/item/delete`）
- **AI 整理分類**：對任意記憶分類點擊「AI 整理此分類」，Gemini 提出改善建議，使用者確認後一鍵覆寫（`POST /api/memory/ai-curate`）
- **KPI 可點擊篩選**：點擊「建議清理」KPI 後，列表僅顯示 `health.status !== 'healthy'` 的問題條目；再次點擊可回到完整列表
- **AI 審查跳轉捷徑**：AI 品質審查結果每筆建議旁新增「跳至分類」捷徑，讓建議可立即 action
