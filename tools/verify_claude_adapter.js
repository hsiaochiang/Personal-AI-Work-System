...
const VERIFY_PORT = Number(process.env.VERIFY_PORT || PORT + 6);
...
async function runAdapterChecks() {
  const transcript = readFixture('claude-share-transcript.txt');
  const transcriptDoc = adaptClaudeConversation(transcript, { inputFormatHint: 'claude-text' });
  ...
  assert.ok(transcriptDoc.messages.every((message) => message.source === 'claude'));
  assert.match(conversationDocToText(transcriptDoc), /Claude adapter/);

  const autoDetectedDoc = adaptConversationInput(transcript);
  validateConversationDoc(autoDetectedDoc);
  assert.strictEqual(autoDetectedDoc.messages[0].source, 'claude');

  assert.deepStrictEqual(memorySourceUtils.getMemorySourcePresentation('claude'), {
    label: 'Claude',
    className: 'source-claude',
  });
  assert.strictEqual(
    memorySourceUtils.buildAttributedMemoryListItem('保留 Claude 來源標記', 'claude'),
    '- 保留 Claude 來源標記 <!-- source: claude -->'
  );
  assert.throws(() => adaptClaudeConversation('僅有一段普通文字'), /Claude transcript/);
...
async function runServerSmoke() {
  ...
  try {
    ...
    assert.match(extractPage.body, /Claude 匯入/);
    assert.match(extractPage.body, /Claude adapter/);

    const adapterScript = await fetchText('/js/conversation-adapters.js');
    assert.strictEqual(adapterScript.status, 200);
    assert.match(adapterScript.body, /adaptClaudeConversation/);
...
async function main() {
  console.log('--- verify_claude_adapter ---');
  await runAdapterChecks();
  await runServerSmoke();
  console.log('ALL PASS claude adapter verification');
}

main().catch((error) => {
  console.error('FAIL claude adapter verification:', error.message);
  process.exit(1);
});
